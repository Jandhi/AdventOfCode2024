use std::{num, ops::{Add, AddAssign, Mul, MulAssign, Sub, SubAssign}};
...
impl<T> Point<T> 
where T : Clone + Copy + Add<Output = T> + AddAssign + Sub<Output = T> + SubAssign + Mul<T, Output = T> + MulAssign<T> + From<i32> + PartialOrd
{
    pub fn new(x: T, y: T) -> Point<T> {
        Point { x, y }
    }

    pub fn cardinal_neighbours(self) -> Vec<Point<T>> {
        vec![
    ...
    pub fn is_in_bounds<TGrid>(self, grid : &Vec<Vec<TGrid>>) -> bool {
        self.x >= T::from(0) && self.y >= T::from(0) && self.x < T::from(grid[0].len() as i32) && self.y < T::from(grid.len() as i32)
    }
}
