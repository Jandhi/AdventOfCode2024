...
pub fn get_ratio<T>(a: Point<T>, b: Point<T>) -> Option<T> 
where T : Clone + Copy + Add<Output = T> + AddAssign + Sub<Output = T> + SubAssign + Mul<T, Output = T> + MulAssign<T> + Div<T, Output = T> + DivAssign<T> + Eq + PartialEq + Rem<T, Output = T> + Default
{
    if a.x % b.x != Default::default() || a.y % b.y != Default::default() {
        return None;
    }
